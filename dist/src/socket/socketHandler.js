    ...
    setupSocketHandlers() {
        this.io.on("connection", async (socket) => {
            ...
            socket.on("join_support_ticket", (ticketId) => {
                ...
                console.log(`User ${auth.username} (${auth.userType}) joined support ticket room: ticket_${ticketId}`);
            });
            ...
            socket.on("send_support_message", async (data) => {
                try {
                    ...
                    console.log(`Broadcasting support message to room: ticket_${ticketId}`);
                    ...
                    console.log(`Support message sent in ticket ${ticketId} by ${auth.username}`);
                }
                catch (error) {
            ...
            socket.on("update_support_ticket_status", async (data) => {
                ...
                try {
                    ...
                    console.log(`Support ticket ${ticketId} status updated by admin ${auth.username}`);
                }
                catch (error) {
            ...
            socket.on("user_closed_ticket", async (data) => {
                ...
                console.log(`Broadcasted user_closed_ticket to admin room for ticket: ${data.ticketId}`);
            });
            socket.on("ticket_auto_close", async (data) => {
                try {
                    ...
                    console.log(`🕒 Ticket ${ticketId} auto-closed due to inactivity (${inactiveMinutes} minutes)`);
                }
                catch (error) {
